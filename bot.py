...
print(f"🚀 vFINAL ACCORDION FIXED - {TODAY.strftime('%d/%m/%Y')}")

WILAYAS = ["الجزائر","المرادية","خميستي","جيجل","شرشال","وهران","قسنطينة","بشار","تندوف","ورقلة","بومرداس","تيبازة","البويرة"]
...
def gen_id(t,s):
    cleaned = re.sub(r'\s+', ' ', t[:200].lower().strip())
    cleaned = cleaned[:120]
    base = cleaned + "|" + s
...
def scrape():
    ...
    print(f"📅 آخر تاريخ: {latest[3]} - عدد التواريخ {len(all_dates)}")
    ...
    for el in soup.find_all(True):
        ...
        if d and len(txt) < 40:
            ...
            continue
        if "طلب العروض" not in txt and "طلب عروض" not in txt:
            continue
        if "2026" not in txt:
            continue
        if len(txt) < 25 or len(txt) > 1200:
            continue
        if txt[:120] in seen:
            continue
        seen.add(txt[:120])
...
for t in new:
    ...
    for i,f in enumerate(nearest,1):
        ...
        murl = f.get('map') or f.get('maps') or f.get('location') or f"https://www.google.com/maps/search/{name}+{t['loc']}"
